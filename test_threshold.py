...
model_forced = ThresholdModel(prefs, min_percentage=0.75, resolution_strategy=ResolutionStrategy.PESSIMISTIC)
...
for i in range(25): complex_prefs.append((f"G1_{i}", np.random.uniform(0.0, 0.2)))

# 0.2-0.4: Sparse (5 people) -> flat (Slope < 1)
# This allows y=x to catch up and cross back (creating unstable eq)
for i in range(5): complex_prefs.append((f"G2_{i}", np.random.uniform(0.2, 0.4)))

# 0.4-0.6: Dense (40 people) -> steep rise again (Slope > 1)
# Should cross y=x again (stable eq)
for i in range(40): complex_prefs.append((f"G3_{i}", np.random.uniform(0.4, 0.6)))

# 0.6-0.8: Sparse (5 people)
for i in range(5): complex_prefs.append((f"G4_{i}", np.random.uniform(0.6, 0.8)))

# 0.8-1.0: Dense (25 people)
for i in range(25): complex_prefs.append((f"G5_{i}", np.random.uniform(0.8, 1.0)))
...
model_comp_opt = ThresholdModel(complex_prefs, resolution_strategy=ResolutionStrategy.OPTIMISTIC)
res_opt = model_comp_opt.resolve(smoothing=0)
print(f"Result: {len(res_opt)} active ({len(res_opt)/100:.2f})")

print("\nResolving from 0.5 (Custom):")
model_comp_custom = ThresholdModel(complex_prefs, resolution_strategy=ResolutionStrategy.CUSTOM)
