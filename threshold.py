...
class ThresholdModel:
    def __init__(self, preferences, resolution_strategy=ResolutionStrategy.PESSIMISTIC, min_percentage=0.0):
        """
    ...
    def display(self, smoothing=None):
        ...
        for r, _ in eqs['stable']:
            plt.plot(r, r, 'go', markersize=8, label='Stable' if 'Stable' not in plt.gca().get_legend_handles_labels()[1] else "")
            plt.annotate(f"{r:.2f}", (r, r), textcoords="offset points", xytext=(0,10), ha='center')
            
        for r, _ in eqs['unstable']:
            plt.plot(r, r, 'mo', markersize=8, label='Unstable' if 'Unstable' not in plt.gca().get_legend_handles_labels()[1] else "")
            plt.annotate(f"{r:.2f}", (r, r), textcoords="offset points", xytext=(0,-15), ha='center')
